            ...
            if let Some(raw_input) = tui::handle_input(&mut app, Duration::from_millis(100))? {
                ...
                match classify_input(&raw_input) {
                    ...
                    InputResult::GameInput(text) => {
                        // Parse intent (uses per-category intent client if configured, else base)
                        let intent_client = app
                            .intent_client
                            .clone()
                            .or_else(|| app.client.clone())
                            .unwrap();
                        let intent_model = if app.intent_model.is_empty() {
                            app.model_name.clone()
                        } else {
                            app.intent_model.clone()
                        };
