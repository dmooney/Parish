...
| 3 | Player Input | **Partial** (~83% of slash commands referenced, ~12 are unit-only) | Per-category `reaction` (`/model.reaction` etc.) untested (**P0**); frontend slash-command registry only 1/32 (**P0**); `IntentKind::Interact` has no local-parse test (**P0**) | [03-player-input.md](regression/03-player-input.md) |
