...
mod tests {
    ...
    // The server process stays alive (Railway shows Online) but every CF-proxied
    // request 502s.  This test exercises the full verify path so the missing
    // feature is caught in CI, not in production (cf. Cloudflare Ray
    // 9f391ab7df46c64a, 2026-04-28T21:19:18Z).
